...
/**
 * PDFBox-version-independent base for {@code ImageElement}. Holds the image
 * data plus its width/height/maxWidth/absolute-position state — all of which
 * are identical between the pdfbox flavours. The PDFBox-coupled
 * {@code draw(...)} and the {@link Dividable#divide(float, float)} (which
 * builds module-specific {@code VerticalSpacer}/{@code Cutter} instances) stay
 * in the subclass.
 */
public abstract class AbstractImageElement
	implements Element, IDrawable, WidthRespecting {
