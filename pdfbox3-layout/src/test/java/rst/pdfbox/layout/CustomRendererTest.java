...
public class CustomRendererTest extends AbstractExampleTest
{
  ...
  {
    final String text1 = "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, " +
    ...
    final String text2 = "At *vero eos et accusam* et justo duo dolores et ea rebum." +
                         "Stet clita kasd gubergren, no sea takimata\n\n" +
    ...
    final Document document = new Document (40, 60, 40, 60);
  ...
  public static class SectionRenderer implements IRenderer, IRenderListener
  {

    private int sectionNumber;

    @Override
    public boolean render (final RenderContext renderContext, final IElement element, final ILayoutHint layoutHint)
                                                                                                                    throws IOException
    {
      if (element instanceof Section)
      {
        ...
        final IElement ruler = new HorizontalRuler (Stroke.builder ()
                                                          .lineWidth (2)
    ...
    {
      final String content = "Section " + sectionNumber + ", Page " + (renderContext.getPageIndex () + 1);
      final TextFlow text = TextFlowUtil.createTextFlow (content,
                                                         11,
                                                         new PDType1Font (Standard14Fonts.FontName.TIMES_ROMAN));
      final float offset = renderContext.getPageFormat ().getMarginLeft () +
                           TextSequenceUtil.getOffset (text, renderContext.getWidth (), EAlignment.Right);
      text.drawText (renderContext.getContentStream (), new Position (offset, 30), EAlignment.Right, null);
    }
  ...
  public static class Section extends Paragraph
  {
    ...
    public Section (final int number) throws IOException
    {
      m_nNumber = number;
