...
{
  /**
   * Represents a highlight annotation that might be added to a {@link AnnotatedStyledText}.
   */
  public static class HighlightAnnotation implements IAnnotation
  {
    private final Color m_aColor;

    public HighlightAnnotation (final Color color)
    {
      this.m_aColor = color;
    }
  ...
  /**
   * Processes {@link HighlightAnnotation}s by adding a colored highlight to the pdf.
   */
  public static class HighlightAnnotationProcessor implements AnnotationProcessor
  {
    @Override
    public void annotatedObjectDrawn (final IAnnotated drawnObject,
                                      final IDrawContext drawContext,
                                      final Position upperLeft,
                                      final float width,
                                      final float height) throws IOException
    {
      final Iterable <HighlightAnnotation> HighlightAnnotations = drawnObject.getAnnotationsOfType (HighlightAnnotation.class);

      for (final HighlightAnnotation highlightAnnotation : HighlightAnnotations)
      {

        // use PDF text markup to implement the highlight
        final PDAnnotationTextMarkup markup = new PDAnnotationHighlight ();

        // use the bounding box of the drawn object to position the
        // highlight
        final PDRectangle bounds = new PDRectangle ();
        ...
        quadPoints = CompatibilityHelper.transformToPageRotation (quadPoints, drawContext.getCurrentPage ());
    ...
    @Override
    public void beforePage (final IDrawContext drawContext) throws IOException
    {
      // nothing to do here for us
    }

    @Override
    public void afterPage (final IDrawContext drawContext) throws IOException
    {
      // nothing to do here for us
    }

    @Override
    public void afterRender (final PDDocument document) throws IOException
    {
      // nothing to do here for us
    }
  ...
  /**
   * The control character is a representation of the parsed markup. It contains any information
   * passed by the markup necessary for rendering, in our case here it is just the color for the
   * highlight.
   */
  public static class HighlightControlCharacter extends AbstractAnnotationControlCharacter <HighlightAnnotation>
  {
  ...
  /**
   * Provides a regex pattern to match the highlight markup, and creates an appropriate control
   * character. In our case here the markup syntax is either <code>{hl}</code> or with optional
   * color information <code>{hl:#ee22aa}</code>, where the color is given as hex RGB code (ee22aa
   * in this case). It can be escaped with a backslash ('\').
   */
                                                        ...
                                                        IAnnotationControlCharacterFactory <HighlightControlCharacter>
  {
    ...
    private final static Pattern PATTERN = Pattern.compile ("(?<!\\\\)(\\\\\\\\)*\\{hl(:#(\\p{XDigit}{6}))?\\}");

    private final static String TO_ESCAPE = "{";

    @Override
    public HighlightControlCharacter createControlCharacter (final String text,
                                                             final Matcher matcher,
                                                             final List <CharSequence> charactersSoFar)
    {
      Color color = null;
      final String hex = matcher.group (3);
      if (hex != null)
      {
  ...
  public void test () throws Exception
  {
    ...
    final Document document = new Document (PageFormat.with ().A4 ().margins (40, 60, 40, 60).portrait ().build ());

    Paragraph paragraph = new Paragraph ();
    paragraph.addText ("Hello there, here is ", 10, new PDType1Font (Standard14Fonts.FontName.HELVETICA));

    // now add some annotated text using our custom highlight annotation
    final HighlightAnnotation annotation = new HighlightAnnotation (Color.green);
    final AnnotatedStyledText highlightedText = new AnnotatedStyledText ("highlighted text",
                                                                         10,
    ...
    final OutputStream outputStream = new FileOutputStream ("customannotation.pdf");
